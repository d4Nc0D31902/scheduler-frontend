                  ...
                  <div className="col-lg-8">
                    <div className="p-5">
                      <div className="d-flex justify-content-between align-items-center mb-5">
                        <h1 className="fw-bold mb-0 text-black">PRE-ORDERED ITEMS</h1>
                      </div>
                      <hr className="my-4" />
                      {cartItems.length === 0 ? (
                        <p className="text-center">Your cart is empty. <Link to="/store">Go back to shopping</Link></p>
                      ) : (
                        <>
                          {cartItems.map((item) => (
                            <Fragment key={item.product}>
                              <div className="row mb-4 d-flex justify-content-between align-items-center">
                                <div className="col-md-2 col-lg-2 col-xl-2">
                                  <img src={item.image} className="img-fluid rounded-3" alt="Product" />
                                </div>
                      ...
                      {cartItems.length > 0 && (
                        <>
                          <div className="d-flex justify-content-between mb-4">
                            <h5 className="text-uppercase">{cartItems.length} MERCHANDISE </h5>
                            <h5>₱ {cartItems.reduce((acc, item) => acc + item.quantity * item.price, 0).toFixed(2)}</h5>
                          </div>
                          <hr className="my-4" />
                          ...
                          <button type="button" className="btn btn-dark btn-block btn-lg" data-mdb-ripple-color="dark" onClick={checkoutHandler}>PROCESS ORDER</button>
                        </>
                      )}
                      {cartItems.length === 0 && (
                        <p className="text-danger text-center">Your cart is empty. Please add merchandise first.</p>
                      )}
                    </div>
                  </div>
                </div>
